...
;Variables
  ...
  !define PRODUCT_NAME "Xazab Electrum"
...
;Interface Settings
  ...
  !define MUI_ICON "electrum_xazab\gui\icons\electrum-xazab.ico"
...
Section "${PRODUCT_NAME}" SectionDE
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\xazab"
  ...
  File /r "dist\electrum-xazab\*.*"
  File "electrum_xazab\gui\icons\electrum-xazab.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-xazab-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-xazab-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-xazab-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-xazab-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-xazab-${PRODUCT_VERSION}.exe" 0


  ;Links xazab: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\xazab" "" "URL:xazab Protocol"
  WriteRegStr HKCU "Software\Classes\xazab" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\xazab" "DefaultIcon" "$\"$INSTDIR\electrum-xazab.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\xazab\shell\open\command" "" "$\"$INSTDIR\electrum-xazab-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-xazab.ico"
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "Xazab Electrum Wallet"
