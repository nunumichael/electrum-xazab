...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias electrum.xazab.xyz -keyalg RSA -keysize 2048 \
        -validity 10000
...
SDIST_NAME_PATTERN = re.compile('^Xazab-Electrum-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Xazab-Electrum-{version}'
PPA_SOURCE_NAME = 'electrum-xazab'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_XAZAB(_Testnet)?-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Xazab-Electrum{testnet}-{version}-release.apk'
