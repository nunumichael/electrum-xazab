...
from .xazab_msg import (DSPoolStatusUpdate, DSMessageIDs, ds_msg_str,
                       ds_pool_state_str, XazabDsaMsg, XazabDsiMsg, XazabDssMsg)
from .xazab_tx import str_ip, CTxIn, CTxOut
...
class PSMixSession:
    ...
    async def run_peer(self):
        ...
        self.xazab_peer = await self.xazab_net.run_mixing_peer(self.peer_str,
                                                             self.sml_entry,
                                                             self)
        if not self.xazab_peer:
    ...
    async def send_dsa(self, pay_collateral_tx):
        ...
        await self.xazab_peer.send_msg('dsa', msg.serialize())
    ...
    async def read_next_msg(self, denominate_wfl, timeout=None):
        ...
        if not res:  # xazab_peer is closed
    ...
    def on_dsq(self, dsq):
        ...
        # signature verified in xazab_peer on receiving dsq message for session
