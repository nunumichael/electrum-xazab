...
from electrum_xazab.util import UserCancelled
from electrum_xazab.keystore import bip39_normalize_passphrase
from electrum_xazab.bip32 import BIP32Node, convert_bip32_path_to_list_of_uint32
from electrum_xazab.logging import Logger
from electrum_xazab.plugin import runs_in_hwd_thread
from electrum_xazab.plugins.hw_wallet.plugin import HardwareClientBase, HardwareHandlerBase
    ...
    def callback_PassphraseRequest(self, req):
        if self.creating_wallet:
            msg = _("Enter a passphrase to generate this wallet.  Each time "
                    ...
                    "access the Xazab coins in the wallet.").format(self.device)
        else:
...
class KeepKeyClientBase(HardwareClientBase, GuiMixin, Logger):
    ...
    def has_usable_connection_with_device(self):
        try:
            res = self.ping("electrum-xazab pinging device")
            assert res == "electrum-xazab pinging device"
