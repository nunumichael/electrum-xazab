...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('apiv2.bitcoinaverage.com',
                                   '/indices/local/ticker/XAZAB%s' % ccy)
...
class Poloniex(ExchangeBase):
    async def get_rates(self, ccy):
        ...
        quote_currencies = {}
        xazab_ticker = json.get('BTC_XAZAB')
        quote_currencies['BTC'] = Decimal(xazab_ticker['last'])
        return quote_currencies


class CoinMarketCap(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinmarketcap.com', '/v1/ticker/xazab/')
        quote_currencies = {}
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/xazab/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/xazab/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/xazab/market_chart?vs_currency=%s&days=max' % ccy)
