...
from electrum_xazab import mnemonic
from electrum_xazab.xazab_ps_util import (filter_log_line, PSLogSubCat, PSStates,
                                        PS_DENOMS_VALS)
from electrum_xazab.i18n import _
from electrum_xazab.util import InvalidPassword
...
class XazabSpinBox(QSpinBox):

    def paintEvent(self, event):
        super(XazabSpinBox, self).paintEvent(event)
        ...
        painter.drawText(textRect, Qt.AlignRight | Qt.AlignVCenter, 'Xazab')
...
class ShowPSKsSeedDlg(WindowModalDialog):
    ...
    def __init__(self, parent, seed, passphrase):
        WindowModalDialog.__init__(self, parent, ('Xazab Electrum - %s %s' %
                                                  (_('PS Keystore'),
                                                   _('Seed'))))
...
class PSDialog(QDialog, MessageBoxMixin):
    ...
    def add_mixing_tab(self):
        ...
        self.keep_amount_sb = XazabSpinBox()
    ...
    def add_denoms_tab(self):
        ...
        self.keep_amount_sb2 = XazabSpinBox()
        ...
        for i, d in enumerate(PS_DENOMS_VALS[::-1]):
            d_le = QLineEdit(self.format_amount(d) + ' XAZAB')
    ...
    def update_keep_amount(self):
        ...
        if psman.calc_denoms_method == psman.CalcDenomsMethod.ABS:
            self.keep_amount_lb.setText(f'{psman.keep_amount} XAZAB')
            self.keep_amount_lb2.setText(f'{psman.keep_amount} XAZAB')
