...
from electrum_xazab import xazab_tx
from electrum_xazab.bitcoin import COIN, is_b58_address, b58_address_to_hash160
from electrum_xazab.xazab_tx import TxOutPoint, service_to_ip_port
from electrum_xazab.protx import ProTxMN, ProTxService, ProRegTxExc
from electrum_xazab.util import bfh, bh2u, FILE_OWNER_MODE
...
class ImportLegacyWizardPage(QWizardPage):
    ...
    def update_lmn_data(self, current):
        ...
        if prevout_hash:
            val_xazab = '%s XAZAB' % (value/COIN) if value else ''
            self.collateral_val.setText(val_xazab)
...
class BlsKeysWizardPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        if start_id in parent.UPD_ENTER_PAGES:
            self.setSubTitle(_('Regenerate BLS keypair, setup xazabd'))
            if not self.bls_priv.text():
                self.bls_priv.setText(new_mn.bls_privk)
                self.bls_pub.setText(new_mn.pubkey_operator)
        else:
            self.setSubTitle(_('Generate BLS keypair, setup xazabd'))
    ...
    def generate_bls_keypair(self):
        ...
        self.bls_info_label.setText(_('BLS keypair generated. Before '
                                      ...
                                      'line to ~/.xazabcore/xazab.conf and '
                                      'restart masternode:'))
