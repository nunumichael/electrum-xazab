...
from electrum_xazab.storage import WalletStorage, StorageReadWriteError
from electrum_xazab.wallet_db import WalletDB
from electrum_xazab.wallet import Wallet, InternalAddressCorruption, Abstract_Wallet
from electrum_xazab.wallet import update_password_for_directory

from electrum_xazab.plugin import run_hook
from electrum_xazab import util
from electrum_xazab.util import (profiler, InvalidPassword, send_exception_to_crash_reporter,
                           format_satoshis, format_satoshis_plain, format_fee_satoshis)
from electrum_xazab.invoices import PR_PAID, PR_FAILED
from electrum_xazab import blockchain
from electrum_xazab.network import Network, TxBroadcastError, BestEffortRequestFailed
from electrum_xazab.interface import PREFERRED_NETWORK_PROTOCOL, ServerAddr
...
#Factory.register('InstallWizard', module='electrum_xazab.gui.kivy.uix.dialogs.installwizard')
#Factory.register('InfoBubble', module='electrum_xazab.gui.kivy.uix.dialogs')
#Factory.register('OutputList', module='electrum_xazab.gui.kivy.uix.dialogs')
#Factory.register('OutputItem', module='electrum_xazab.gui.kivy.uix.dialogs')
...
from .uix.dialogs.xazab_kivy import TorWarnDialog
...
# the data
Cache.register('electrum_xazab_widgets', timeout=0)
...
Factory.register('TabbedCarousel', module='electrum_xazab.gui.kivy.uix.screens')
...
from electrum_xazab.util import (NoDynamicFeeEstimates, NotEnoughFunds,
...
if TYPE_CHECKING:
    ...
    from electrum_xazab.plugin import Plugins
    from electrum_xazab.paymentrequest import PaymentRequest
...
class ElectrumWindow(App, Logger):
    ...
    def on_qr(self, data):
        from electrum_xazab.bitcoin import is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
    ...
    def scan_qr(self, on_complete):
        ...
        PythonActivity = autoclass('org.kivy.android.PythonActivity')
        SimpleScannerActivity = autoclass("org.xazab.electrum.qr.SimpleScannerActivity")
        Intent = autoclass('android.content.Intent')
    ...
    def on_start(self):
        ...
        # bind intent for xazab: URI scheme
        if platform == 'android':
            from android import activity
            from jnius import autoclass
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
    ...
    def xazab_net_dialog(self):
        from .uix.dialogs.xazab_net import XazabNetDialog
        if self._xazab_net_dialog is None:
            self._xazab_net_dialog = XazabNetDialog(self)
        self._xazab_net_dialog.update()
        self._xazab_net_dialog.open()
    ...
    def init_ui(self):
        ...
        Factory.register('AnimatedPopup',
                         module='electrum_xazab.gui.kivy.uix.dialogs')
        Factory.register('QRCodeWidget',
                         module='electrum_xazab.gui.kivy.uix.qrcodewidget')

        # preload widgets. Remove this if you want to load the widgets on demand
        #Cache.append('electrum_xazab_widgets', 'AnimatedPopup', Factory.AnimatedPopup())
        #Cache.append('electrum_xazab_widgets', 'QRCodeWidget', Factory.QRCodeWidget())
        ...
        if self.testnet:
            self.icon = os.path.dirname(KIVY_GUI_PATH) + "/icons/electrum-xazab-testnet.png"
        else:
            self.icon = os.path.dirname(KIVY_GUI_PATH) + "/icons/electrum-xazab.png"
    ...
    def get_max_amount(self, is_ps=False):
        from electrum_xazab.transaction import PartialTxOutput
