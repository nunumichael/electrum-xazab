...
from electrum_xazab import util

from electrum_xazab.gui.kivy.i18n import _

from electrum_xazab.gui.kivy.uix.dialogs.question import Question


Builder.load_string('''
#:import _ electrum_xazab.gui.kivy.i18n._
#:import MIN_PEERS_LIMIT electrum_xazab.xazab_net.MIN_PEERS_LIMIT
#:import MAX_PEERS_LIMIT electrum_xazab.xazab_net.MAX_PEERS_LIMIT


<XazabNetStatItem@SettingsItem>
    total: 0
    received: 0
    sent: 0
    ...
    description: _('Data flow over Xazab network')


<XazabNetDataFlowDialog@Popup>
    title: _('Data flow over Xazab network')
    data: data
    ScrollView:
        Label:
            id: data
...
<XazabNetDialog@Popup>
    title: _('Xazab Network')
    id: dlg
    BoxLayout:
        orientation: 'vertical'
        ScrollView:
            GridLayout:
                ...
                CardSeparator
                SettingsItem:
                    value: ': ON' if root.run_xazab_net else ': OFF'
                    title: _('Enable Xazab Network') + self.value
                    description: _('Enable or Disable Xazab network')
                    action: root.toggle_xazab_net
                CardSeparator
                XazabNetStatItem
                    total: root.total
                ...
                SettingsItem:
                    ...
                    description: _('Number of currently connected Xazab peers')
                    action: root.show_peers
                CardSeparator
                SettingsItem:
                    title: _('Max Peers') + ': %s' % root.max_peers
                    description: _('Maximally allowed Xazab peers count')
                ...
                SettingsItem:
                    ...
                    description: _('Xazab Sporks Values')
                    action: root.show_sporks
                CardSeparator
                SettingsItem:
                    title: _('Banlist') + ': %s' % len(root.banlist)
                    description: _('Banned Xazab Peers')
...
class XazabNetDataFlowDialog(Factory.Popup):
    ...
    def open(self, *args, **kwargs):
        super(XazabNetDataFlowDialog, self).open(*args, **kwargs)
        util.register_callback(self.update_cb,
                               ['xazab-net-activity',
                                'xazab-peers-updated'])

    def dismiss(self, *args, **kwargs):
        super(XazabNetDataFlowDialog, self).dismiss(*args, **kwargs)
    ...
    def update(self):
        res = ''
        peers = self.dn_dlg.xazab_net.peers
...
class XazabPeerCard(Factory.BoxLayout):
...
class ConnectedPeersPopup(Factory.Popup):
    ...
    def update(self, *args, **kwargs):
        ...
        self.vbox.add_widget(XazabPeerCard(_('Peer'),
                                          _('User Agent'),
                                          is_title=True))
        for peer, ua in self.dn_dlg.peers:
            self.vbox.add_widget(XazabPeerCard(peer, ua))
...
class MaxPeersPopup(Factory.Popup):
    ...
    def __init__(self, dn_dlg):
        Factory.Popup.__init__(self)
        self.dn_dlg = dn_dlg
        self.slider.value = dn_dlg.xazab_net.max_peers

    def dismiss(self, value=None):
        super(MaxPeersPopup, self).dismiss()
        if value is not None:
            self.dn_dlg.xazab_net.max_peers = value
...
class StaticPeersPopup(Factory.Popup):
    ...
    def __init__(self, dn_dlg):
        Factory.Popup.__init__(self)
        self.dn_dlg = dn_dlg
        self.edit.text = dn_dlg.xazab_net.xazab_peers_as_str()

    def dismiss(self, xazab_peers=None):
        if xazab_peers is None:
            super(StaticPeersPopup, self).dismiss()
            return

        net = self.dn_dlg.net
        xazab_net = net.xazab_net
        res = xazab_net.xazab_peers_from_str(xazab_peers)
        if type(res) == str:
            self.err_label.text = f'Error: {res}'
        else:
            super(StaticPeersPopup, self).dismiss()
            self.dn_dlg.config.set_key('xazab_peers', res, True)
            net.run_from_another_thread(xazab_net.set_parameters())
            self.dn_dlg.static_peers = xazab_net.xazab_peers_as_str()
...
class BanlistCard(Factory.BoxLayout):
    ...
    def on_remove(self, peer):
        if not peer:
            return
        xazab_net = self.dn_dlg.xazab_net
...
class BlsSpeedPopup(Factory.Popup):
    ...
    def __init__(self, dn_dlg):
        Factory.Popup.__init__(self)
        self.xazab_net = dn_dlg.xazab_net
        self.clock_e = Clock.schedule_once(self.update, 0.5)

    def update(self, *args, **kwargs):
        start_t = time.time()
        res = self.xazab_net.test_bls_speed()
...
class XazabNetDialog(Factory.Popup):
    ...
    def __init__(self, app):
        ...
        self.xazab_net = self.net.xazab_net
        Factory.Popup.__init__(self)
    ...
    def update(self):
        self.on_xazab_net_activity()
        self.on_sporks_activity()
        self.on_xazab_peers_updated()
        self.on_xazab_banlist_updated()
        self.on_mn_list_diff_updated()
        self.on_network_updated()
        self.run_xazab_net = self.config.get('run_xazab_net', True)
        self.max_peers = self.xazab_net.max_peers
        self.use_static_peers = self.config.get('xazab_use_static_peers', False)
        self.static_peers = self.xazab_net.xazab_peers_as_str()

    def open(self, *args, **kwargs):
        super(XazabNetDialog, self).open(*args, **kwargs)
        util.register_callback(self.on_xazab_net_activity_cb,
                               ['xazab-net-activity'])
        util.register_callback(self.on_sporks_activity_cb,
                               ['sporks-activity'])
        util.register_callback(self.on_xazab_peers_updated_cb,
                               ['xazab-peers-updated'])
        util.register_callback(self.on_xazab_banlist_updated_cb,
    ...
    def dismiss(self, *args, **kwargs):
        super(XazabNetDialog, self).dismiss(*args, **kwargs)
        util.unregister_callback(self.on_xazab_net_activity_cb)
        util.unregister_callback(self.on_sporks_activity_cb)
        util.unregister_callback(self.on_xazab_peers_updated_cb)
        util.unregister_callback(self.on_xazab_banlist_updated_cb)
    ...
    def on_xazab_net_activity_cb(self, event, *args):
        Clock.schedule_once(lambda dt: self.on_xazab_net_activity())

    def on_xazab_net_activity(self):
        read_bytes = self.xazab_net.read_bytes
        write_bytes = self.xazab_net.write_bytes
    ...
    def on_sporks_activity(self):
        sporks_dict = self.xazab_net.sporks.as_dict()
        self.sporks = []
    ...
    def on_xazab_peers_updated_cb(self, event, *args):
        Clock.schedule_once(lambda dt: self.on_xazab_peers_updated())

    def on_xazab_peers_updated(self):
        self.peers = []
        for peer, xazab_peer in self.xazab_net.peers.items():
            ua = xazab_peer.version.user_agent.decode('utf-8')
            self.peers.append((peer, ua))

    def on_xazab_banlist_updated_cb(self, event, *args):
        Clock.schedule_once(lambda dt: self.on_xazab_banlist_updated())

    def on_xazab_banlist_updated(self):
        banlist = self.xazab_net.banlist
        self.banlist = []
    ...
    def toggle_xazab_net(self, *args):
        self.run_xazab_net = not self.config.get('run_xazab_net', True)
        self.config.set_key('run_xazab_net', self.run_xazab_net, True)
        self.net.run_from_another_thread(self.net.xazab_net.set_parameters())
    ...
    def toggle_use_static_peers(self, *args):
        use_static_peers = not self.config.get('xazab_use_static_peers', False)
        self.use_static_peers = use_static_peers
        self.config.set_key('xazab_use_static_peers', use_static_peers, True)
        net = self.net
    ...
    def show_data_flow(self, *args):
        XazabNetDataFlowDialog(self).open()
