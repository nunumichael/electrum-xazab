...
from electrum_xazab.xazab_msg import (XazabVersionMsg, XazabDsaMsg, XazabDssuMsg,
                                    XazabDsqMsg, XazabDsiMsg, XazabDsfMsg,
                                    XazabDssMsg, XazabDscMsg)
from electrum_xazab.xazab_tx import TxOutPoint, CTxIn, CTxOut
from electrum_xazab.transaction import Transaction
from electrum_xazab.util import bfh, bh2u

from . import TestCaseForTestnet


class TestXazabMsg(TestCaseForTestnet):

    def test_version_msg(self):
        msg = XazabVersionMsg.from_hex(VERSION_MSG)
        ...
        assert msg.user_agent == b'/Xazab Core:0.14.0.3/'
    ...
    def test_dsa_msg(self):
        msg = XazabDsaMsg.from_hex(DSA_MSG)
        assert msg.nDenom == 2
    ...
    def test_dssu_msg(self):
        msg = XazabDssuMsg.from_hex(DSSU_MSG)
    ...
    def test_dsq_msg(self):
        msg = XazabDsqMsg.from_hex(DSQ_MSG)
        assert msg.nDenom == 2
    ...
    def test_dsi_msg(self):
        msg = XazabDsiMsg.from_hex(DSI_MSG)
    ...
    def test_dsc_msg(self):
        msg = XazabDscMsg.from_hex(DSC_MSG)
        assert msg.sessionID == 67305985
        assert msg.messageID == 21
