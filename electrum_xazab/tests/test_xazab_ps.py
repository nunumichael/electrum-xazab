...
from electrum_xazab import xazab_ps, ecc
from electrum_xazab.address_synchronizer import (TX_HEIGHT_LOCAL,
                                                TX_HEIGHT_UNCONF_PARENT,
                                                TX_HEIGHT_UNCONFIRMED)
from electrum_xazab.bitcoin import COIN
from electrum_xazab.xazab_ps_util import (COLLATERAL_VAL, CREATE_COLLATERAL_VAL,
...
from electrum_xazab.xazab_ps_wallet import (KPStates, KP_ALL_TYPES, KP_SPENDABLE,
                                          KP_PS_COINS, KP_PS_CHANGE,
                                          PSKsInternalAddressCorruption)
from electrum_xazab.xazab_tx import PSTxTypes, SPEC_TX_NAMES
from electrum_xazab import keystore
from electrum_xazab.simple_config import SimpleConfig
from electrum_xazab.storage import WalletStorage
from electrum_xazab.transaction import (Transaction, PartialTxOutput,
                                       PartialTransaction)
from electrum_xazab.util import Satoshis, NotEnoughFunds, TxMinedInfo, bh2u
from electrum_xazab.wallet import Wallet
...
class PSWalletTestCase(TestCaseForTestnet):
    ...
    def test_calc_need_denoms_amounts_on_keep_amount(self):
        w = self.wallet
        psman = w.psman
        two_xazab_amnts_val = 200142001

        res = psman.calc_need_denoms_amounts()
        assert sum([sum(amnts)for amnts in res]) == two_xazab_amnts_val
        ...
        assert sum([c.value_sats() for c in coins]) == 50000000  # 0.5 Xazab
    ...
    def test_cache_keypairs_group_origin_by_addr(self):
        ...
        psman.state = PSStates.Mixing
        psman._cache_keypairs(password=None)
        cache_results = [5, 3, 187, 4760, 300]
        for i, cache_type in enumerate(KP_ALL_TYPES):
            assert len(psman._keypairs_cache[cache_type]) == cache_results[i]
        psman._cleanup_all_keypairs_cache()
        assert psman._keypairs_cache == {}
    ...
    def test_prepare_funds_from_hw_wallet(self):
        ...
        assert sum([c.value_sats() for c in coins]) == 350002000  # 3.5 Xazab
